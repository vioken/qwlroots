...
void QWSeatPrivate::on_pointer_grab_begin(void *data)
{
    ...
    Q_EMIT q_func()->pointer_grab_begin();
}

void QWSeatPrivate::on_pointer_grab_end(void *data)
{
    Q_ASSERT(m_handle == data);
    Q_EMIT q_func()->pointer_grab_end();
}

void QWSeatPrivate::on_touch_grab_begin(void *data)
{
    Q_ASSERT(m_handle == data);
    Q_EMIT q_func()->touch_grab_begin();
}

void QWSeatPrivate::on_touch_grab_end(void *data)
{
    Q_ASSERT(m_handle == data);
    Q_EMIT q_func()->touch_grab_end();
}

void QWSeatPrivate::on_keyboard_grab_begin(void *data)
{
    Q_ASSERT(m_handle == data);
    Q_EMIT q_func()->keyboard_grab_begin();
}

void QWSeatPrivate::on_keyboard_grab_end(void *data)
{
    Q_ASSERT(m_handle == data);
    Q_EMIT q_func()->keyboard_grab_end();
}

void QWSeatPrivate::on_request_set_cursor(void *data)
{
    Q_EMIT q_func()->request_set_cursor(reinterpret_cast<wlr_seat_pointer_request_set_cursor_event*>(data));
}

void QWSeatPrivate::on_request_set_selection(void *data)
{
    Q_EMIT q_func()->request_set_selection(reinterpret_cast<wlr_seat_request_set_selection_event*>(data));
}

void QWSeatPrivate::on_set_selection(void *data)
{
    Q_ASSERT(m_handle == data);
    Q_EMIT q_func()->set_selection();
}

void QWSeatPrivate::on_request_set_primary_selection(void *data)
{
    Q_EMIT q_func()->request_set_primary_selection(reinterpret_cast<wlr_seat_request_set_primary_selection_event*>(data));
}

void QWSeatPrivate::on_set_primary_selection(void *data)
{
    Q_ASSERT(m_handle == data);
    Q_EMIT q_func()->set_primary_selection();
}

void QWSeatPrivate::on_request_start_drag(void *data)
{
    Q_EMIT q_func()->request_start_drag(reinterpret_cast<wlr_seat_request_start_drag_event*>(data));
}

void QWSeatPrivate::on_start_drag(void *data)
{
    Q_EMIT q_func()->start_drag(reinterpret_cast<wlr_drag*>(data));
}
